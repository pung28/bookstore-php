                ...
                <ul id="iq-sidebar-toggle" class="iq-menu">
                    ...
                    <li>
                        ...
                        <ul id="admin" class="iq-submenu collapse" data-parent="#iq-sidebar-toggle">
                            ...
                            <li><a href="index.php?controller=book" ><i class="ri-book-2-line"  ></i>Sách</a></li>
                            <li><a href="index.php?controller=book_detail" aria-expanded="true"><i class="ri-book-2-line"></i>Chi tiết sách</a></li>
                    ...
                    <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                            <div class="iq-header-title">
                                <h4 class="card-title">Sửa chi tiết sách</h4>
                            </div>
                        </div>
                        <div class="iq-card-body">
                            <?php
                            foreach ($array['book_details'] as $book_detail){
                                ?>
                                <form method="post" action="index.php?controller=book_detail&action=update" enctype="multipart/form-data">
                                    <input type="hidden" name="id" value="<?= $book_detail['id']?>" <br>
                                    <div class="form-group">
                                        <label>Ảnh:</label>
                                        <?php
                                        if (!empty($book_detail['part'])){
                                            ?>
                                            Image: <input type="file" name="bd_image"><br>
                                            <img src="<?= $book_detail['image'] ?>" width="200px"><br>
                                            <input type="hidden" name="current_image" value="<?= $book_detail['image'] ?>">
                                        <?php
                                        } else {
                                            ?>
                                            <input type="file" name="bd_image"><br>
                                            <?php
                                        }
                                        ?>
                                    ...
                                    <div class="form-group">
                                        <label>Giá:</label>
                                        <input type="text" name="price" value="<?= $book_detail['price']?>" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>Số lượng:</label>
                                        <input type="text" name="quantity" value="<?= $book_detail['quantity']?>" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>Năm xuất bản:</label>
                                        <input type="text" name="publishing_year" value="<?= $book_detail['publishing_year']?>" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>Nhà xuất bản:</label>
                                        <input type="text" name="publishing_company" value="<?= $book_detail['publishing_company']?>" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>Mô tả:</label>
                                        <input type="text" name="description" value="<?= $book_detail['description']?>" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>Trạng thái:</label>
                                        <select name="status" class="form-control" id="exampleFormControlSelect1">
                                            <option value="1" <?php if ($book_detail['status'] == 1) { echo 'selected'; } ?>>Còn hàng</option>
                                            <option value="0" <?php if ($book_detail['status'] == 0) { echo 'selected'; } ?>>Hết hàng</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>Tập:</label>
                                        <input type="text" name="part" value="<?= $book_detail['part']?>" class="form-control">
                                    </div>
                                    <div class="form-group">
                                        <label>Tên sách:</label>
                                        <select name="book_id" class="form-control" id="exampleFormControlSelect2">
                                            <?php foreach ($array['books'] as $book): ?>
                                                <option value="<?= $book['id'] ?>" <?php if ($book['id'] == $book_detail['book_id']) echo 'selected'; ?>>
                                                    <?= $book['name'] ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="version_id" class="form-control" id="exampleFormControlSelect2">
                                            <?php foreach ($array['versions'] as $version): ?>
                                                <option value="<?= $version['id'] ?>" <?php if ($version['id'] == $book_detail['version_id']) echo 'selected'; ?>>
                                                    <?= $version['name'] ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
    ...
    <div class="iq-colorbox color-fix">
        ...
        <div id="right-sidebar-scrollbar" class="iq-colorbox-inner">
            <div class="clearfix color-picker">
                <h3 class="iq-font-black">KBookstore Awesome Color</h3>
